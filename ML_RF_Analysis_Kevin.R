...
###############################   Genetic Clusters    ########################################################
###############if you want to include the Shoot Root and Seed data############################################
##############################################################################################################
only_data<-data[,c(5,12:ncol(data))]
colnames(only_data)
only_data[1]<- as.character(only_data$Cluster.9)
##############################################################################################################
###############if you want to exclude the Shoot Root and Seed data############################################
##############################################################################################################
only_data<-data[,c(5,15:ncol(data))]
colnames(only_data)
only_data[1]<- as.character(only_data$Cluster.9)
...
###############if you want to include the Shoot Root and Seed data############################################
##############################################################################################################
only_data<-data[,c(7,12:53)]
colnames(only_data)
only_data[1]<- as.factor(only_data$tSNE_k2_day9)
##############################################################################################################
###############if you want to exclude the Shoot Root and Seed data############################################
##############################################################################################################
only_data<-data[,c(6,15:53)]
colnames(only_data)
only_data[1]<- as.character(only_data$tSNE_k2_day6)
##############################################################################################################
##############################################################################################################
...
set.seed(11)
indexes<-createDataPartition(y$tSNE_k2_day9, # the outcome variable
...
set.seed(9)
caret.rf <- train(tSNE_k2_day9 ~ ., 
                   data = y,
                   method = "rf", 
                   tuneLength = 10,
                   trControl = rf.train.control,importance=T,ntree=200)
...
plot(rf_variable_importance,top=12)
...
rf_variable_importance<-varImp(caret.rf)
rf_var_importance<-print(rf_variable_importance)
plot(rf_variable_importance,top=10)
a<-testSet[,1]
plot(a,rf_preds)
cor(a,rf_preds)
